...
<table>
	...
	<tr>
		<td>상품 목록 페이지<br>Header/Footer<br>판매자 센터<br>프로젝트 세팅</td>
		<td>로그인 페이지<br>모달 컴포넌트<br>테스트 및 품질 관리</td>
		<td>회원가입 페이지<br>(구매자/판매자)<br>장바구니 페이지</td>
		<td>상품 상세 페이지<br>전체 페이지<br>CSS 검수 및 수정</td>
	</tr>
</table>
...
### 8.1 [협업] Git 브랜치 충돌 해결

**문제**: 4명이 동시에 작업하다 보니 `api.js`, `Header.js` 등 공통 파일에서 머지 충돌이 빈번하게 발생
...
**교훈**: 충돌을 피하려 하기보다, 충돌을 잘 해결하는 방법을 익히는 게 더 중요함. 오히려 충돌 해결 과정에서 서로의 코드를 이해하게 됨.

---

### 8.2 [협업] 코드 리뷰를 통한 XSS 취약점 발견

**문제**: PR 코드 리뷰 중 `innerHTML`로 사용자 데이터를 직접 삽입하는 XSS 취약점 발견

**원인**: 각자 개발할 때는 보안 이슈를 인지하지 못했으나, 다른 팀원이 코드를 보면서 문제 발견

**해결**: 팀 전체가 모여 XSS 개념 학습 후, 21개 파일을 분담하여 `textContent`/DOM API로 리팩토링
```javascript
// 리뷰에서 지적된 코드
...
**교훈**: 혼자 개발할 때는 놓치기 쉬운 문제도 코드 리뷰를 통해 발견할 수 있음. 코드 리뷰는 단순 검수가 아니라 팀 전체의 학습 기회.

---

### 8.3 [Vite] MPA 동적 라우팅 처리
...
### 8.8 [비동기] ID 중복확인 Race Condition
...
### 8.10 [배포] Netlify 환경에서 CSS 로드 실패
...
### 9.1 [보안] innerHTML을 DOM API로 전환 (XSS 취약점 방지)
...
### 9.2 [스타일] CSS !important 남용 문제 해결
...
### 김유진
- 이번 프로젝트에서 회원가입과 장바구니 기능을 주로 담당하며, 프론트엔드 개발의 흐름을 실제로 체감할 수 있었습니다. 회원가입 기능을 구현하며 입력값 검증, 에러 메시지 처리, 사용자 경험을 고려한 흐름 설계의 중요성을 느꼈고, 장바구니 개발 과정에서는 상태 변화에 따라 UI가 어떻게 반응해야 하는지, 그리고 서버와의 통신 결과를 화면에 자연스럽게 반영하는 패턴을 익힐 수 있었습니다.
이번 프로젝트를 통해 프론트엔드 개발이 화면 구현에 그치지 않고, 데이터 흐름과 사용자 경험을 함께 설계하는 작업이라는 점을 분명히 이해하게 되었고, 앞으로 더 구조적인 코드를 작성하고 싶다는 목표도 생겼습니다.
